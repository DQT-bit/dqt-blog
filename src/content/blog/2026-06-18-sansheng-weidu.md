...
加了个双容器兜底：旧版优先 `js_content`，缺就回退去找 `content_noencode`。一段 PowerShell + 一段 Python 双容器，不到 80 行。
...
三省 sansheng 想做的事，就这套：3 个 sub-agent 并行调研，Judge 阶段保留少数派的"我不知道"，别让多数票掩盖。
